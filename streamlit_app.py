...
def plot_points_route(points, order=None, title=""):
    # points: dict name -> (x,y)
    ...
    fig.update_layout(
        ...
        yaxis=dict(zeroline=False, showgrid=True, gridcolor="rgba(255,255,255,0.08)", scaleanchor="x", scaleratio=1),
...
if generate or st.session_state.scenario is None:
    if mission == 1:
        st.session_state.scenario = rx.preparar_tsp(int(seed), int(stars))
        st.session_state.meta = {"type": "points", "points": st.session_state.scenario["entregas"], "tam": st.session_state.scenario["tam"]}
    elif mission == 2:
        st.session_state.scenario = rx.preparar_cvrp(int(seed), int(stars))
        st.session_state.meta = {"type": "points", "points": st.session_state.scenario["entregas"], "tam": st.session_state.scenario["tam"]}
    elif mission == 3:
        st.session_state.scenario = rx.preparar_vrptw(int(seed), int(stars))
        st.session_state.meta = {"type": "points", "points": st.session_state.scenario["entregas"], "tam": st.session_state.scenario["tam"]}
    elif mission == 4:
        st.session_state.scenario = rx.preparar_mochila(int(seed), int(stars))
        st.session_state.meta = {"type": "knapsack"}
    elif mission == 5:
        st.session_state.scenario = rx.preparar_dijkstra(int(seed), int(stars))
        st.session_state.meta = {"type": "graph"}
    elif mission == 6:
        st.session_state.scenario = rx.preparar_flota(int(seed), int(stars))
        st.session_state.meta = {"type": "points", "points": st.session_state.scenario["entregas"], "tam": st.session_state.scenario["tam"]}
    elif mission == 7:
        st.session_state.scenario = rx.preparar_orienteering(int(seed), int(stars))
        st.session_state.meta = {"type": "points", "points": st.session_state.scenario["entregas"], "tam": st.session_state.scenario["tam"]}
...
with col_left:
    if meta["type"] == "points":
        points = meta["points"]
        st.plotly_chart(plot_points_route(points, None, title=f"Misión {mission} — ★{'★'*stars}"), use_container_width=True)
        ...
        st.write("Aristas:")
        st.json({"-".join(sorted(tuple(k))): w for k, w in scenario["aristas"].items()})
        # Quick plot (edges + nodes) using plotly
        fig = go.Figure()
        for key, w in scenario["aristas"].items():
            a, b = tuple(key)
            xa, ya = pts[a]
            xb, yb = pts[b]
            fig.add_trace(go.Scatter(x=[xa, xb], y=[ya, yb], mode="lines", line=dict(color="rgba(200,200,200,0.25)", width=2), showlegend=False))
        fig.add_trace(go.Scatter(x=[p[0] for p in pts.values()], y=[p[1] for p in pts.values()], mode="markers+text", text=list(pts.keys()), textposition="top center", marker=dict(size=12, color="#ffcc00", line=dict(width=1, color="#111")), showlegend=False))
        fig.update_layout(template="plotly_dark", height=650, margin=dict(l=20, r=20, t=40, b=20), title=f"Misión 5 — ★{'★'*stars}", yaxis=dict(scaleanchor="x", scaleratio=1))
...
with col_right:
    ...
    if mission in (1, 3):
        st.write("Escribe el orden, ej: `A C B D` (usa todas exactamente una vez)")
        inp = st.text_input("Orden", value="")
        if st.button("Evaluar"):
            order = parse_tokens(inp)
            if set(order) != set(scenario["entregas"].keys()) or len(order) != len(scenario["entregas"]):
                st.error("Orden inválido: debes usar cada punto exactamente una vez.")
            else:
                if mission == 1:
                    d = rx.longitud_ruta(order, scenario["entregas"])
                    nn = rx.ruta_vecino_mas_cercano(scenario["entregas"])
                    dnn = rx.longitud_ruta(nn, scenario["entregas"])
                    score = rx.puntuar_min(d, dnn)
                    st.metric("Puntuación", f"{score}/100")
                    st.metric("Distancia", f"{d:.2f}")
                    st.plotly_chart(plot_points_route(scenario["entregas"], order, title=f"Tu ruta — {score}/100"), use_container_width=True)
                else:
                    dist, tard, _ = rx._sim_vrptw(order, scenario["entregas"], scenario["ventanas"], scenario["velocidad"])
                    costo = dist + scenario["pen"] * tard
                    edd = sorted(scenario["entregas"].keys(), key=lambda k: scenario["ventanas"][k][1])
                    distb, tardb, _ = rx._sim_vrptw(edd, scenario["entregas"], scenario["ventanas"], scenario["velocidad"])
        ...
        if st.button("Evaluar"):
            ...
            if len(all_nodes) != len(set(all_nodes)):
            ...
            elif set(all_nodes) != set(scenario["entregas"].keys()):
                ...
                for i, v in enumerate(viajes, 1):
                    ...
                    if load > cap:
                        ...
                        st.stop()
                dist = sum(rx.longitud_ruta(v, scenario["entregas"]) for v in viajes)
                base = rx._heuristica_cvrp(scenario["entregas"], scenario["demanda"], scenario["capacidad"])
                distb = sum(rx.longitud_ruta(v, scenario["entregas"]) for v in base)
        ...
        st.write(f"Escribe camino de {scenario['origen']} a {scenario['destino']}, ej: `A B C`")
        ...
        if st.button("Evaluar"):
            ...
            if not path or path[0] != scenario["origen"] or path[-1] != scenario["destino"]:
                ...
                st.metric("Puntuación", f"{score}/100")
                st.write({"costo": cost, "optimo": opt_cost, "opt_path": opt_path})

    elif mission == 6:
        st.write("Una línea por vehículo. Ej:\nA C\nB D E")
        ...
        if st.button("Evaluar"):
            ...
            k = scenario["k"]
            if len(grupos) != k:
                st.error(f"Debes ingresar exactamente {k} líneas (una por vehículo).")
            else:
                ...
                if len(all_nodes) != len(set(all_nodes)):
                    ...
                    largos = rx._largos_grupos(grupos, scenario["entregas"])
                    makes = max(largos)
                    base = rx._heuristica_flota(scenario["entregas"], scenario["k"])
                    makesb = max(rx._largos_grupos(base, scenario["entregas"]))
                    score = rx.puntuar_min(makes, makesb)
                    st.metric("Puntuación", f"{score}/100")
                    st.write({"makespan": makes, "baseline": makesb, "largos": largos})

    elif mission == 7:
        st.write("Escribe entregas a visitar en orden, ej: `A C E` (puede ser vacío)")
        inp = st.text_input("Orden", value="")
        if st.button("Evaluar"):
            order = parse_tokens(inp)
            if len(order) != len(set(order)):
                st.error("No repitas entregas.")
            elif not set(order) <= set(scenario["entregas"].keys()):
                st.error("Entregas inválidas.")
            else:
                dist = rx.longitud_ruta(order, scenario["entregas"]) if order else 0.0
                if dist > scenario["presupuesto"] + 1e-9:
                    ...
                    opt_val, _, _ = rx._orienteering_optimo(
                        scenario["entregas"], scenario["premio"], scenario["presupuesto"], int(stars)
                    )
